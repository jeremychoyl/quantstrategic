...
export const MILESTONES: Milestone[] = [
  ...
  {
    week: "W9", date: "2026-05-28", category: "infra",
    title: "/session command",
    items: ["Current-session summary card: range, volume, net Δ, CVD drift, VWAP distance","Bias line: net buying/selling with ▲/▼ bar counts","Completes the read set: /levels (where) + /delta (recent) + /session (narrative)"],
  },
  {
    week: "W10", date: "2026-05-28", category: "infra",
    title: "Session comparison + log rotation",
    items: ["/session appends previous-session comparison (range, net Δ, bias)","Webhook log rotation: 30-day retention (~4.3MB cap)","CVD-cumulative caveat documented — net Δ is the only valid cross-session metric"],
  },
  {
  ...
  {
    week: "W18", date: "2026-05-28", category: "infra",
    title: "Watchdog fix + /recap",
    items: ["Watchdog false-positive fixed (open-time-aware STALE_MINUTES 10→13)","/recap: 24h regime history grouped by session with ⚡ transition markers","Brief error handling: Telegram alerts on data / prompt-build failures"],
  },
  {
    week: "W19", date: "2026-06-01", category: "infra",
    title: "History dedup + brief prompt hardening",
    items: ["Regime history dedup by bar_time (brief + 5-min crons collided at same minute)","Brief prompt: use only real levels, banned invented pivot/support/resistance terms","First 4-day data snapshot assessed (882 bars, IMBALANCED_UP 64%)"],
  },
  {
  ...
  {
    week: "W74", date: "2026-07-01", category: "research",
    title: "SMC frontier closed + CI & auto-deploy",
    items: ["Smart Money Concepts explored → 0 deployable: FVG post-2021-only / WF-fail, liquidity-sweep 0/12 (every config net-losing — NQ continues, doesn't revert)","VIX / volatility futures screened → rejected for $0: not Tradovate-tradeable (CFE-only), no both-era data (2018+), −0.63 corr = redundant risk-off axis","EMA-short re-checked vs deployed legs → passes the full gate suite (both-era PF 1.45, WF OOS 1.47 / 0 sit-outs, 4-leg book Sharpe 1.28→1.53); a validated candidate — deploy is a capital/ops call","ORB-EMA correlation RE-corrected to +0.01 / +0.05 (the +0.43 was a raw-ORB measurement artifact) — all three live legs are diversifiers, no concentration","GitHub Actions CI (tests on push) + Vercel git auto-deploy connected — quantstrategic now ships on git push"],
  },
]
