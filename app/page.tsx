    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 space-y-6">
        ...
        {data?.shadows && data.shadows.items.length > 0 && (
          <div className="mt-6">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <h2 className="font-bold" style={{ fontSize: "1.3125rem" }}>🕵️ Shadow Tests</h2>
              <span className="font-bold px-2 py-0.5 rounded uppercase tracking-wide"
                    style={{ fontSize: "0.9375rem", background: "#1a2230", color: "var(--muted)", border: "1px solid var(--border)" }}>
                Paper · review-gated
              </span>
              {data.shadows.next_review && (
                <span className="font-semibold px-2 py-0.5 rounded tabular-nums"
                      style={{ fontSize: "0.9375rem", background: "var(--surface)", color: "var(--muted)", border: "1px solid var(--border)" }}>
                  Next review {data.shadows.next_review}
                </span>
              )}
            </div>
            <p className="mb-3" style={{ fontSize: "1.125rem", color: "var(--muted)" }}>{data.shadows.note}</p>
            <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
              {data.shadows.items.map((it, i) => (
                <div key={i} className="px-3 py-2 border-b last:border-b-0"
                     style={{ fontSize: "1.125rem", borderColor: "var(--border)" }}>
